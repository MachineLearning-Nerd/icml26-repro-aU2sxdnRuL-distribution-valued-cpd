...
Through the pre-release research nodes, 24 HF runs used 6,230 seconds (1 h 43 m 50 s) of listed wall time. At the official `cpu-upgrade` price of `$0.0005/min` (`$0.03/h`), the duration-based estimate is `$0.0519`; final candidate validation will be added before publication. Billing is provider-metered by minute, so this is an evidence-based estimate rather than an invoice.
